...
void encrypt(std::string filename, std::vector<std::complex<double>> vec) {
    ...
    */
    ...
    // std::cout << "Available Slots: " << numSlots << std::endl;
    
    if (vec.size() > numSlots) {
        std::cerr << "Error: Vector size (" << vec.size() << ") exceeds available slots (" << numSlots << ")." << std::endl;
        std::exit(1);
    }
...
int main(int argc, char* argv[]) {
    ...
    if (argc == 3) {
        ...
        std::cout << out[numWeights-1].real();
