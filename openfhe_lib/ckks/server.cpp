...
void load_client_encrypted_weights() {
    ...
    */
    ...
    for (int i = 1; i <= 5; ++i) {
        std::string path = DATAFOLDER + "/enc_weight_client" + std::to_string(i) + ".txt";
        Ciphertext<DCRTPoly> enc;
        if (!Serial::DeserializeFromFile(path, enc, SerType::BINARY)) {
            std::cerr << "I cannot read serialized data from: " << path << std::endl;
            std::exit(1);
        }
        enc_weights.push_back(enc);
    }
...
int main(int argc, char* argv[]) {
    ...
    // perform aggregation
    double n_clients = 4;
    aggregator(enc_weights, n_clients);
    return 0;
}